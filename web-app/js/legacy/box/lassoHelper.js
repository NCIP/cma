...
function writePendings()	{
	...
	if (pendingSamples.length == 0){
		...
		document.getElementById("sampleCount").innerHTML = pendingSamples.length + " smaples selected";
	}
}
...
var main = new dBox("pcaChart");
main.box = true;
main.color = "red";
main.verbose = true;
////////////The following code is specific to TCGA
var char = document.getElementById("pcaChart");
var myMapArray = char.useMap.split("#");
///////////////////
var theAreas;
...
function startup() {
	...
	function didSelectAnything(name, minx, miny, maxx, maxy)	{
		...
		var gotem = "";
		//The following code no longer needed for TCGA
		//get the map
...
function initMarkerPoints(myMap)	{
	...
	//get the areas
	//var theAreas = theMap[0].getElementsByTagName("area");
	///////////The following code changed for TCGA
...
startup();
function setMain(newImage){
	//Two browsers use different ways to draw.
	//Following code makes both of them work in TCGA page only.
	var plot = document.getElementById(newImage);
	myMapArray = plot.useMap.split("#");
	var browser=navigator.appName;
	var b_version=navigator.appVersion;
	var version=parseFloat(b_version);
	if (browser=="Microsoft Internet Explorer" && (version>=4))
	{
		main.anchor.src=plot.src;
	}
	else{
		main = new dBox(newImage);
		main.box = true;
		main.color = "red";
		main.verbose = true;
		startup();
