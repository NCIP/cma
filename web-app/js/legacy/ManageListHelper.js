		 	...
		 	try	{
				...
				wDiv.style.width="400px";
		 		if(items.length > 0)	{
					...
					for(var i=0; i<items.length; i++)	{
						...
						itemNotes = "";	
							if(items[i].rank!=null){
								itemRank = " <b style='color:black'>rank:</b> " + items[i].rank;
							}
							if(items[i].notes!=null){
								itemNotes = " <b style='color:black'>notes:</b> <span style='font-style:italic'>" + items[i].notes + "</span>";
							}						
						if(listType == "Gene"){
							tmp += "<li id='"+listName + itemId + "_div"+"' class='detailsList'>"+(i+1) +") "  + " <a href=\"http://cgwb-dev.nci.nih.gov/cgi-bin/gene2?gene=" + itemId + "\" target='_blank'>"+ itemId + "</a> " + itemRank + itemNotes;	
						}
						else {
					...
					wDiv.innerHTML += "<div onclick=\"location.href='manageLists/export?list="+eid+"';\" style='margin:20px;cursor:pointer; width:90px;height:20px'><img src='../images/downArrow20.png'/><u>export list</u></div>";
				}
				else{
			    	document.getElementById(listName + "details").appendChild(dDIV);
